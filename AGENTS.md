...
| Classi layout copy | `lib/ui-classes.ts` — `appPageShell`, **`appPageShellCharacter`** (scheda XL), `appPanelStack`, `appCanvas`, **`appSelectField` / `appFormControl`** (touch ~48px mobile, MD3), `appFieldLabel` / `appFieldHint`, `appMuted`, … |
